...
const InitPuzzle = (
  ...
  //South East First
  console.log('NorthPipe', NorthPipe);
  console.log('SouthPipe', SouthPipe);
  console.log('EastPipe', EastPipe);
  console.log('WestPipe', WestPipe);
  let nextCardinalDirection: cardinals = 'S';
  let possibilities = [];
  if (NorthPipeLeadsToS && SouthPipeLeadsToS) {
    // Taking South Pipe
    console.log('Taking South Pipe');
...
const FollowPipe = (
  ...
  Puzzle.push({ x: newX, y: newY });
  return FollowPipe(newX, newY, input, PipeMap, Puzzle, nextCardinal);
};
...
const Part1 = (input: string): number => {
  ...
  const steps =
    solutionMap.length % 2 === 0
      ? solutionMap.length / 2
      : (solutionMap.length + 1) / 2;
  return steps;
};
